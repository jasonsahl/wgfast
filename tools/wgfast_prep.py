...
def main(matrix):
    ...
    subprocess.check_call("raxmlHPC-SSE3 -f d -p 12345 -m ASC_GTRGAMMA -s out.fasta -n nasp --no-bfgs > /dev/null 2>&1", shell=True)
    subprocess.check_call("raxmlHPC-SSE3 -f e -m ASC_GTRGAMMA -s out.fasta -t RAxML_bestTree.nasp -n PARAMS --no-bfgs > /dev/null 2>&1", shell=True)
