...
def main(matrix, model):
    ...
    if model == "ASC_GTRGAMMA":
        subprocess.check_call("raxmlHPC-SSE3 -f d -p 12345 -m %s -s out.fasta -n nasp --asc-corr=lewis > /dev/null 2>&1" % model, shell=True)
        subprocess.check_call("raxmlHPC-SSE3 -f e -m %s -s out.fasta -t RAxML_bestTree.nasp -n PARAMS --asc-corr=lewis > /dev/null 2>&1" % model, shell=True)
    else:
        subprocess.check_call("raxmlHPC-SSE3 -f d -p 12345 -m %s -s out.fasta -n nasp > /dev/null 2>&1" % model, shell=True)
        subprocess.check_call("raxmlHPC-SSE3 -f e -m %s -s out.fasta -t RAxML_bestTree.nasp -n PARAMS > /dev/null 2>&1" % model, shell=True)
...
if __name__ == "__main__":
    ...
    parser.add_option("-o", "--model", dest="model",
                      ...
                      action="store", type="string", default="ASC_GTRGAMMA")
