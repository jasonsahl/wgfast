...
try:
    ...
    sys.exit()
import glob
import wgfast.igs_logging as log_isg
    ...
    if os.path.isfile("%s.tmp.xyx.matrix" % idx):
        ...
        if len(f)>1:
            if os.path.isfile("%s.F.paired.fastq.gz" % idx):
                ...
                try:
                    subprocess.check_call("bbduk.sh in=%s in2=%s ref=%s out=%s.F.paired.fastq.gz out2=%s.R.paired.fastq.gz minlen=%s overwrite=true > /dev/null 2>&1" % (
                        f[0], f[1], bin_path('illumina_adapters_all.fasta'), idx, idx, length), shell=True)
            ...
            length = int(get_sequence_length(f[0])/2)
            try:
                subprocess.check_call("bbduk.sh in=%s ref=%s out=%s.single.fastq.gz minlen=%s overwrite=true ignorebadquality> /dev/null 2>&1" % (f[0],bin_path('illumina_adapters_all.fasta'),idx,length), shell=True)
            except Exception as e:
                print("Read trimmer did not finish correctly", e)
                sys.exit()
            if os.path.isfile("%s_renamed_header.bam" % idx):
...
def create_params_files(id, to_prune_set, full_tree, full_matrix, dist_sets, processors):
    ...
    for item in to_prune_set:
        ...
        if os.path.isfile("%s-PARAMS" % new_name):
            ...
            for line in open("%s.tmp.tree" % new_name):
...
def process_temp_matrices_dev(dist_sets, sample, tree, processors, patristics, insertion_method, parameters, model):
    ...
    if os.path.isfile("%s.tree_including_unknowns_noedges.tree" % full_context):
        ...
        for line in open("%s.tmp.tree" % full_context):
            if line.startswith("[&U]"):
                fields = line.split()
                fixed_fields = [ ]
                for x in fields:
    ...
    if os.path.isfile("%s.resampling_distances.txt" % full_context):
        ...
        try:
            ...
            for line in open("%s.resampling_distances.txt" % full_context):
